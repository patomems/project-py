...
for benchmark in benchmarks:
    benchmark_prices = yf.download(benchmark, period='max')['Adj Close']
...
correlation_matrix = pd.concat([returns, yf.download(etf_tickers, period='max')['Adj Close'].pct_change().dropna()], axis=1).corr()
